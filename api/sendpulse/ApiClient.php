...
class ApiClient implements ApiInterface
{
    ...
    protected function sendRequest($path, $method = 'GET', $data = array(), $useToken = true)
    {
		...
		static::log('SendPulse request', [
			'path' => $path,
			'httpMethod' => $method,
			'data' => $data,
			'useToken' => $useToken,
			'response' => json_decode(json_encode($retval), true),
		]);
	...
	{
		return LANG_ROOT_PATH . '/logs/sendpulse/' . date('Y-m-d') . '.log';
	}

	protected static function getLogErrorFilePath()
	{
		return LANG_ROOT_PATH . '/logs/sendpulse/errors/' . date('Y-m-d') . '.log';
	}

	protected static function deleteOldLogs()
	{
		if ($directory = opendir(LANG_ROOT_PATH . '/logs/sendpulse/'))
		{
			...
			while (false !== ($file = readdir($directory)))
			{
				if ($file != "." && $file != ".." && $file != '.htaccess')
				{
					...
					if ($diffDays > $timeLife && !is_dir(LANG_ROOT_PATH . '/logs/sendpulse/' . $file))
					{
						unlink(LANG_ROOT_PATH . '/logs/sendpulse/' . $file);
					}
				}
			}
			closedir($directory);
